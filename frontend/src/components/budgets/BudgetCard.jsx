  ...
  return (
    <Card className="rounded-[30px] border-slate-200/80 bg-white/95 p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Orcamento mensal</p>
          <h3 className="mt-2 text-2xl font-semibold tracking-tight text-slate-900">{budget.name}</h3>
          <p className="mt-2 text-sm text-slate-500">{budget.category?.name || 'Categoria nao informada'}</p>
        </div>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <div>
          ...
          <p className="mt-2 text-lg font-semibold text-slate-900">{formatCurrencyBRL(budget.amount)}</p>
        </div>
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Utilizado</p>
          <p className="mt-2 text-lg font-semibold text-slate-900">{formatCurrencyBRL(budget.usedAmount)}</p>
        </div>
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Restante</p>
          <p className="mt-2 text-lg font-semibold text-slate-900">{formatCurrencyBRL(budget.remainingAmount)}</p>
        </div>
      </div>
      ...
      <div className="mt-6">
        <div className="mb-2 flex items-center justify-between gap-3 text-sm text-slate-500">
          ...
          <span className="font-medium text-slate-800">{formatPercentage(budget.usedPercentage)}</span>
        </div>
